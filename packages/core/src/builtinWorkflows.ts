...
/**
 * The single source of truth for built-in / system workflows.
 *
 * These were previously defined in TWO places that had silently drifted apart:
 *   - apps/web (full 14-entry Workflow catalog, used to render the library UI)
 *   - apps/api (a 3-entry {id,title,prompt_md} subset of the "assistant"
 *     workflows, used to seed the chat workflow store)
 *
 * Two copies of the same legal-prompt data is a correctness hazard: an edit to
 * one prompt (e.g. the CP-checklist generation instructions) would not reach the
 * other consumer. This module holds the catalog ONCE, framework-free, so both
 * apps derive from the same data. The API side re-derives its {id,title,prompt_md}
 * shape from the `assistant` entries here (see apps/api/src/lib/builtinWorkflows.ts).
 */
...
export const BUILTIN_WORKFLOW_IDS = new Set(
    BUILTIN_WORKFLOWS.map((wf) => wf.id),
);
