...
Global (measured 2026-08 over the widened `src/**` scope): **41.63%
statements / 35.61% branches / 43.78% functions / 42.99% lines**; the ratchet
floors in `backend/vitest.config.mts` sit below those. The global
number is dominated by a few very large feature libs (toolDispatcher,
documentOps, courtlistener) that are still untested.
