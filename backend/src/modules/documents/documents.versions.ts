...
export async function deleteVersion(
    ...
    | {
          ok: false;
          kind: "doc_not_found" | "version_not_found" | "only_version" | "db";
          detail: string;
      }
> {
    const access = await ensureDocumentAccess(documentId, userId, userEmail, db, {
    ...
    const { data: versions, error: versionsErr } = await db
        ...
        .is("deleted_at", null);
    if (versionsErr) {
        return { ok: false, kind: "db", detail: versionsErr.message };
    }
    ...
    if (doc.current_version_id === versionId) {
        const { error: updateErr } = await db
            ...
            .eq("id", documentId);
        if (updateErr) {
            return { ok: false, kind: "db", detail: updateErr.message };
        }
    }
    ...
    const { error: deleteErr } = await db
        ...
        .is("deleted_at", null);
    if (deleteErr) {
        return { ok: false, kind: "db", detail: deleteErr.message };
    }
