...
// terminal for the request (500 with the install error's message).
async function ensureDefaultsForRequest(
  ...
  res.status(500).json({ detail: result.detail });
  return false;
}
...
workflowsRouter.get("/system", requireAuth, asyncRoute(async (req, res) => {
  const workflowType =
    typeof req.query.type === "string" && req.query.type
      ? req.query.type
      : null;
  res.json(listSystemWorkflows(workflowType));
}));
...
workflowsRouter.use(
  routerErrorHandler("[workflows]", "Failed to process workflow request"),
);
