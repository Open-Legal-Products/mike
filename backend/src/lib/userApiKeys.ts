...
export async function getUserApiKeys(userId: string, client: Db = defaultDb): Promise<UserApiKeys> {
...
export async function saveUserApiKey(
  ...
  if (!normalized) {
    await client
      ...
      .where(and(eq(user_api_keys.user_id, userId), eq(user_api_keys.provider, provider)));
    return;
  }
