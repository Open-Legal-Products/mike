...
import type { StreamChatParams, StreamChatResult, NormalizedToolCall } from "./types";
...
export async function streamGemini(params: StreamChatParams): Promise<StreamChatResult> {
  const {
    model,
    systemPrompt,
    tools = [],
  ...
  for (let iter = 0; iter < maxIter; iter++) {
    const stream = await ai.models.generateContentStream({
      ...
      config: {
        ...
        tools: functionDeclarations.length ? [{ functionDeclarations } as never] : undefined,
        ...
        thinkingConfig: enableThinking ? { includeThoughts: true } : { thinkingBudget: 0 },
      },
    });
    ...
    for await (const chunk of stream) {
      const parts =
        (chunk as { candidates?: { content?: { parts?: GeminiPart[] } }[] }).candidates?.[0]
          ?.content?.parts ?? [];
