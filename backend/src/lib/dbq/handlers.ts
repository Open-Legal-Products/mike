...
//   workflow.catalog_sync    — re-sync the workflow add-on catalog (deduped,
//                      so N replicas booting at once produce ONE sync)
...
/**
 * Re-sync the workflow add-on catalog from the generated module.
 *
 * WHY a job: the sync is a multi-KB upsert plus a reference-file walk, and
 * every replica used to run it inline at boot — N replicas, N identical
 * syncs, all racing the same rows. Enqueued under a fixed dedupe key it
 * happens exactly once per deploy no matter how many processes start.
 *
 * Idempotent: the sync skips rows whose content_hash already matches and
 * upserts the rest, so a retry after a partial run is a no-op on what
 * landed. Throws on failure, which is the retry signal.
 */
export async function handleWorkflowCatalogSync(db: Db): Promise<void> {
    // The job IS the "sync now" request, so ignore any once-per-process latch
    // an earlier request in this process may have set (WORKERS_MODE=inline
    // shares a module registry with the routes). Going back through
    // syncWorkflowAddonCatalog re-latches, so a co-located request path still
    // sees the warm no-op it sees today.
    resetCatalogSyncLatch();
...
export const DB_JOB_HANDLERS: DbJobHandlers = {
    ...
    "workflow.catalog_sync": handleWorkflowCatalogSync,
