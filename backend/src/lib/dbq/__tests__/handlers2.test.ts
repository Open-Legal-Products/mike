// Coverage for the three kinds added after the export work: the deduped
// workflow catalog sync, the proactive MCP OAuth refresh, and the legacy
// Office text precompute. Kept in its own file so the export handlers' mock
// surface (which stubs storage wholesale) stays untouched.

import { describe, it, expect, vi, beforeEach } from "vitest";

const syncWorkflowAddonCatalog = vi.fn(async (..._a: unknown[]) => {});
const resetCatalogSyncLatch = vi.fn(() => {});
vi.mock("../../workflowCatalog", async (importOriginal) => {
    const actual = await importOriginal<typeof import("../../workflowCatalog")>();
    return {
        ...actual,
        syncWorkflowAddonCatalog: (...a: unknown[]) =>
            syncWorkflowAddonCatalog(...a),
        resetCatalogSyncLatch: () => resetCatalogSyncLatch(),
    };
});
...
beforeEach(() => {
    syncWorkflowAddonCatalog.mockReset().mockResolvedValue(undefined);
    resetCatalogSyncLatch.mockReset();
...
describe("handleWorkflowCatalogSync", () => {
    it("drops the once-per-process latch and runs the sync", async () => {
        await handleWorkflowCatalogSync(DB, JOB("workflow.catalog_sync", {}));
        // Order matters: latching first would make the sync a no-op.
        expect(resetCatalogSyncLatch).toHaveBeenCalledTimes(1);
        expect(syncWorkflowAddonCatalog).toHaveBeenCalledTimes(1);
        expect(syncWorkflowAddonCatalog).toHaveBeenCalledWith(DB);
        expect(resetCatalogSyncLatch.mock.invocationCallOrder[0]).toBeLessThan(
            syncWorkflowAddonCatalog.mock.invocationCallOrder[0],
        );
    });

    it("propagates a sync failure so the job retries", async () => {
        syncWorkflowAddonCatalog.mockRejectedValueOnce(new Error("upsert 503"));
        await expect(
            handleWorkflowCatalogSync(DB, JOB("workflow.catalog_sync", {})),
        ).rejects.toThrow(/upsert 503/);
    });
});
