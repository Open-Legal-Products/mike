...
import { recordChatTurn, type AuditStatus } from "../lib/audit";
...
async function ensureWordDocumentRow(
  ...
  if (error || !data) {
    console.error(
      "[word-chat] failed to resolve document",
      safeErrorLog(error),
    );
    return null;
  }
  ...
  asyncRoute(async (req, res) => {
    ...
    if (!chatId && persistChat) {
      const { data, error } = await db
        ...
        .single();
      if (error || !data) {
        console.error("[word-chat] failed to create chat", safeErrorLog(error));
        return void res
          .status(500)
    ...
    // Word turns belong in the same audit history as web chats, tagged with
    // the "word" surface. Fire-and-forget, exactly like the assistant route:
    // an audit failure must never break the stream. Local-storage mode is the
    // user's explicit "never persist my chats server-side" choice, and an
    // audit row carries a content-derived title — so local turns write no
    // audit row at all.
    const recordTurn = (
      title: string | null,
      events: unknown[] | null,
    ...
    try {
      ...
      if (isAbortError(error)) {
        ...
        await updateChatActivity();
        recordTurn(chatTitle, null, "cancelled");
        return;
      }
      // The user's message was already persisted, so an errored first turn
      // would otherwise leave the chat untitled and sorted by creation time.
      await updateChatActivity();
