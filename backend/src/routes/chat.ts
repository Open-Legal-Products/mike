...
import { chats, chat_messages, document_edits, document_versions, projects } from "../db/schema";
import {
  buildDocContext,
  buildMessages,
...
async function getAccessibleChat(
  ...
  if (row.project_id) {
    const access = await checkProjectAccess(row.project_id, userId, userEmail, client);
...
chatRouter.get("/", requireAuth, async (req, res) => {
  ...
  const whereClause =
    ownProjectIds.length > 0
      ? or(eq(chats.user_id, userId), inArray(chats.project_id, ownProjectIds))
      : eq(chats.user_id, userId);

  const baseQuery = db.select().from(chats).where(whereClause).orderBy(desc(chats.created_at));
...
chatRouter.post("/create", requireAuth, async (req, res) => {
  ...
  const projectAccess = await validateAccessibleProjectId(projectId, userId, userEmail, db);
  if (!projectAccess.ok)
