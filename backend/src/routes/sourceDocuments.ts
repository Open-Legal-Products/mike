...
// not an upstream outage, so it answers 400 rather than 502. courtlistener.ts
// throws a fixed message when no token is configured and prefixes the upstream
// status when CourtListener refuses the one it was given.
function isCourtlistenerCredentialError(message: string): boolean {
  return (
...
sourceDocumentsRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    try {
      ...
      // Never hand the raw upstream text back: an auth failure echoes the
      // token it was given, and safeErrorMessage redacts it.
      const detail = safeErrorMessage(error, "Failed to load document");
      const status = isCourtlistenerCredentialError(detail) ? 400 : 502;
