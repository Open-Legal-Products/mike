...
tabularRouter.get("/", requireAuth, async (req, res) => {
    ...
    try {
      const rows = await db
        ...
        .where(
          projectIdFilter
            ? and(
                eq(tabular_reviews.user_id, userId),
                eq(tabular_reviews.project_id, projectIdFilter),
              )
            : eq(tabular_reviews.user_id, userId),
        )
        .orderBy(desc(tabular_reviews.created_at));
  ...
  const sharedPromise = (async () => {
    ...
    try {
      const rows = await db
        ...
        .where(
          and(
            inArray(tabular_reviews.project_id, sharedProjectIds),
            ne(tabular_reviews.user_id, userId),
          ),
        )
        .orderBy(desc(tabular_reviews.created_at));
      return { data: rows, error: null as null | Error };
  ...
  const sharedDirectPromise = (async () => {
    ...
    try {
      const rows = await db
        ...
        .where(
          and(
            sql`${tabular_reviews.shared_with} @> ${JSON.stringify([userEmail])}::jsonb`,
            ne(tabular_reviews.user_id, userId),
          ),
        )
        .orderBy(desc(tabular_reviews.created_at));
  ...
  ] = await Promise.all([ownPromise, sharedPromise, sharedDirectPromise]);
  ...
  if (sharedDirectErr)
    console.warn("[tabular] shared-by-email query failed:", sharedDirectErr.message);
  ...
  if (reviewIds.length > 0) {
    const cells = await db
      .select({
        review_id: tabular_cells.review_id,
        document_id: tabular_cells.document_id,
      })
      .from(tabular_cells)
      .where(inArray(tabular_cells.review_id, reviewIds));
    if (cells) {
      const seen = new Set<string>();
...
tabularRouter.post("/", requireAuth, async (req, res) => {
  const userId = res.locals.userId as string;
  const userEmail = res.locals.userEmail as string | undefined;
  const { title, document_ids, columns_config, workflow_id, project_id } = req.body as {
    title?: string;
    document_ids: string[];
  ...
  const allowedDocumentIds = Array.isArray(document_ids)
    ...
    : [];
  let review: typeof tabular_reviews.$inferSelect | undefined;
  try {
    ...
    return void res
      .status(500)
      .json({ detail: (err as Error)?.message ?? "Failed to create review" });
  }
  if (!review) return void res.status(500).json({ detail: "Failed to create review" });
  ...
  if (cells.length) await db.insert(tabular_cells).values(cells);
...
tabularRouter.post("/prompt", requireAuth, async (req, res) => {
  ...
  const format: string = typeof req.body.format === "string" ? req.body.format : "text";
...
tabularRouter.get("/:reviewId", requireAuth, async (req, res) => {
  ...
  const review = await db.query.tabular_reviews.findFirst({
    where: eq(tabular_reviews.id, reviewId),
  });
  if (!review) return void res.status(404).json({ detail: "Review not found" });
  const access = await ensureReviewAccess(review, userId, userEmail, db);
  if (!access.ok) return void res.status(404).json({ detail: "Review not found" });

  const cells = await db.select().from(tabular_cells).where(eq(tabular_cells.review_id, reviewId));
  const docIds = [...new Set((cells ?? []).map((c) => c.document_id))];
  let docsData: Record<string, unknown>[] = [];
  if (docIds.length > 0) {
    docsData = await db.select().from(documents).where(inArray(documents.id, docIds));
...
tabularRouter.get("/:reviewId/people", requireAuth, async (req, res) => {
  ...
  // Same pattern as /projects/:id/people: walk users (mirror of Cognito
  // identities) to map emails to user_ids, then pull display_names from
...
tabularRouter.patch("/:reviewId", requireAuth, async (req, res) => {
  ...
  if (req.body.columns_config != null) updates.columns_config = req.body.columns_config;
  ...
  const existingReview = await db.query.tabular_reviews.findFirst({
    where: eq(tabular_reviews.id, reviewId),
  });
  if (!existingReview) return void res.status(404).json({ detail: "Review not found" });
  ...
  let updatedReview: typeof tabular_reviews.$inferSelect | undefined;
  try {
    const rows = await db
    ...
    return void res.status(500).json({
      detail: (err as Error)?.message ?? "Failed to update review",
    });
  }
  if (!updatedReview) return void res.status(500).json({ detail: "Failed to update review" });

  if (Array.isArray(req.body.columns_config) || Array.isArray(req.body.document_ids)) {
    const existingCells = await db
      .select({
        document_id: tabular_cells.document_id,
        column_index: tabular_cells.column_index,
      })
      .from(tabular_cells)
      .where(eq(tabular_cells.review_id, reviewId));
    const existingKeys = new Set(
      (existingCells ?? []).map((cell) => `${cell.document_id}:${cell.column_index}`),
    );
    ...
    if (Array.isArray(req.body.document_ids)) {
      ...
      if (removedDocIds.length > 0) {
        try {
          await db
            ...
            .where(
              and(
                eq(tabular_cells.review_id, reviewId),
                inArray(tabular_cells.document_id, removedDocIds),
              ),
            );
        } catch (err) {
          return void res.status(500).json({ detail: (err as Error).message });
        }
      }
      ...
      if (documentIds.length === 0 && existingReview.project_id) {
        const projectDocs = await db
          ...
          .from(documents)
          .where(eq(documents.project_id, existingReview.project_id));
    ...
    const activeColumns = Array.isArray(req.body.columns_config)
      ...
      : ((updatedReview.columns_config as { index: number }[] | null) ?? []);
    const newCells = documentIds.flatMap((documentId) =>
      activeColumns
        .filter((column: { index: number }) => !existingKeys.has(`${documentId}:${column.index}`))
    ...
    if (newCells.length > 0) {
      try {
        await db.insert(tabular_cells).values(newCells);
      } catch (err) {
        return void res.status(500).json({ detail: (err as Error).message });
      }
    }
...
tabularRouter.delete("/:reviewId", requireAuth, async (req, res) => {
  ...
  try {
    await db
      ...
      .where(and(eq(tabular_reviews.id, reviewId), eq(tabular_reviews.user_id, userId)));
  } catch (err) {
    return void res.status(500).json({ detail: (err as Error).message });
  }
  res.status(204).send();
  ...
  try {
    await db
      ...
      .where(
        and(
          eq(tabular_cells.review_id, reviewId),
          inArray(tabular_cells.document_id, document_ids),
        ),
      );
  } catch (err) {
    return void res.status(500).json({ detail: (err as Error).message });
...
tabularRouter.post("/:reviewId/regenerate-cell", requireAuth, async (req, res) => {
  ...
  if (!column) return void res.status(400).json({ detail: "Column not found" });

  const docAllowed = await filterAccessibleDocumentIds([document_id], userId, userEmail, db);
  if (docAllowed.length === 0) return void res.status(404).json({ detail: "Document not found" });
  const doc = await db.query.documents.findFirst({
    where: eq(documents.id, document_id),
    columns: { id: true, filename: true, file_type: true },
  });
  ...
  await db
    ...
    .where(
      and(
        eq(tabular_cells.review_id, reviewId),
        eq(tabular_cells.document_id, document_id),
        eq(tabular_cells.column_index, column_index),
      ),
    );
...
tabularRouter.post("/:reviewId/generate", requireAuth, async (req, res) => {
  ...
  const columns: {
    ...
    (review.columns_config as
      ...
      | null) ?? [];
  if (columns.length === 0) return void res.status(400).json({ detail: "No columns configured" });
        ...
        for (const col of columnsToProcess) {
          ...
          if (existingCell) {
            await db
              .update(tabular_cells)
              .set({ status: "generating", content: null })
              .where(eq(tabular_cells.id, existingCell.id as string));
          } else {
            await db.insert(tabular_cells).values({
              review_id: reviewId,
              document_id: docId,
              column_index: col.index,
              status: "generating",
            ...
            async (columnIndex, result) => {
              ...
              await db
                ...
                .where(
                  and(
                    eq(tabular_cells.review_id, reviewId),
                    eq(tabular_cells.document_id, docId),
                    eq(tabular_cells.column_index, columnIndex),
                  ),
                );
              write(
            ...
            await db
              ...
              .where(
                and(
                  eq(tabular_cells.review_id, reviewId),
                  eq(tabular_cells.document_id, docId),
                  eq(tabular_cells.column_index, col.index),
                ),
              );
            write(
...
tabularRouter.get("/:reviewId/chats", requireAuth, async (req, res) => {
  ...
  const chats = await db
    .select({
      id: tabular_review_chats.id,
      title: tabular_review_chats.title,
      created_at: tabular_review_chats.created_at,
      updated_at: tabular_review_chats.updated_at,
      user_id: tabular_review_chats.user_id,
    })
    .from(tabular_review_chats)
    .where(eq(tabular_review_chats.review_id, reviewId))
    .orderBy(desc(tabular_review_chats.updated_at));
...
tabularRouter.delete("/:reviewId/chats/:chatId", requireAuth, async (req, res) => {
  ...
  try {
    await db
      ...
      .where(and(eq(tabular_review_chats.id, chatId), eq(tabular_review_chats.user_id, userId)));
  } catch (err) {
    return void res.status(500).json({ detail: (err as Error).message });
  }
  res.status(204).send();
...
tabularRouter.get("/:reviewId/chats/:chatId/messages", requireAuth, async (req, res) => {
  ...
  const chat = await db.query.tabular_review_chats.findFirst({
    where: eq(tabular_review_chats.id, chatId),
    columns: { id: true, review_id: true },
  });
  ...
  const messages = await db
    .select({
      id: tabular_review_chat_messages.id,
      role: tabular_review_chat_messages.role,
      content: tabular_review_chat_messages.content,
      annotations: tabular_review_chat_messages.annotations,
      created_at: tabular_review_chat_messages.created_at,
    })
    .from(tabular_review_chat_messages)
    .where(eq(tabular_review_chat_messages.chat_id, chatId))
    .orderBy(asc(tabular_review_chat_messages.created_at));
...
tabularRouter.post("/:reviewId/chat", requireAuth, async (req, res) => {
  ...
  if (!reviewAccess.ok) return void res.status(404).json({ detail: "Review not found" });
  ...
  const sortedColumns = ((review.columns_config ?? []) as { index: number; name: string }[]).sort(
    (a, b) => a.index - b.index,
  );
  ...
  const tabularStore: TabularCellStore = {
    ...
    documents: docs,
    cells: new Map(
      (cells ?? []).map((c) => [`${c.column_index}:${c.document_id}`, parseCellContent(c.content)]),
    ),
  };
  ...
  if (chatId) {
    ...
    const existing = await db.query.tabular_review_chats.findFirst({
      where: eq(tabular_review_chats.id, chatId),
      columns: {
        id: true,
        title: true,
  ...
  if (chatId) {
    await db.insert(tabular_review_chat_messages).values({
      chat_id: chatId,
      role: "user",
