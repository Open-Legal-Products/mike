...
import {
  SYSTEM_WORKFLOW_IDS as MIKE_SYSTEM_WORKFLOW_IDS,
  SYSTEM_WORKFLOWS as MIKE_SYSTEM_WORKFLOWS,
  type SystemWorkflow,
} from "../lib/systemWorkflows";
import {
  ROSS_SYSTEM_WORKFLOW_IDS,
  ROSS_SYSTEM_WORKFLOWS,
} from "../lib/rossSystemWorkflows";
import { findMissingUserEmails } from "../lib/userLookup";

export const workflowsRouter = Router();
const SYSTEM_WORKFLOWS = [...MIKE_SYSTEM_WORKFLOWS, ...ROSS_SYSTEM_WORKFLOWS];
const SYSTEM_WORKFLOW_IDS = new Set([
  ...MIKE_SYSTEM_WORKFLOW_IDS,
  ...ROSS_SYSTEM_WORKFLOW_IDS,
]);
...
const DEFAULT_WORKFLOW_PRACTICE = "Civil Litigation";
const DEFAULT_WORKFLOW_JURISDICTIONS = ["Canada / Ontario"];
  ...
  asyncRoute(async (req, res) => {
    ...
    if (type !== "assistant" && type !== "tabular")
      return void res.status(400).json({
        detail: "metadata.type must be 'assistant' or 'tabular'",
      });
