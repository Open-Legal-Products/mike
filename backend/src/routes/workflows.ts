...
import { hidden_workflows, user_profiles, users, workflow_shares, workflows } from "../db/schema";
...
async function loadSharerNames(client: Db, sharerIds: string[]): Promise<Map<string, string>> {
...
workflowsRouter.get(
  ...
  asyncRoute(async (req, res) => {
    ...
    // Own workflows
    const ownConditions = [eq(workflows.user_id, userId), eq(workflows.is_system, false)];
    ...
    if (shares.length > 0) {
      ...
      if (wfs.length > 0) {
        const sharerIds = shares.map((s) => s.shared_by_user_id).filter((id): id is string => !!id);
...
workflowsRouter.post(
  ...
  asyncRoute(async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ detail: err instanceof Error ? err.message : "Insert failed" });
    }
  }),
);
...
async function handleWorkflowUpdate(req: Request, res: Response) {
  ...
  const [row] = await db
    ...
    .returning();
  if (!row) return void res.status(404).json({ detail: "Workflow not found or not editable" });
  res.json(
...
workflowsRouter.delete(
  ...
  asyncRoute(async (req, res) => {
    ...
    await db
      ...
      .where(
        and(eq(hidden_workflows.user_id, userId), eq(hidden_workflows.workflow_id, workflowId)),
      );
    res.status(204).send();
  }),
);
...
workflowsRouter.delete(
  ...
  asyncRoute(async (req, res) => {
    ...
    await db
      ...
      .where(and(eq(workflow_shares.id, shareId), eq(workflow_shares.workflow_id, workflowId)));
    res.status(204).send();
  }),
);
