...
export function startAllWorkers(): void {
    ...
    //
    // Through the queue with a fixed dedupe key, so a deploy that starts ten
    // replicas produces ONE sync instead of ten racing upserts. If the
    // enqueue itself cannot reach the database, fall back to exactly the old
    // inline behavior — a warm catalog matters more than a tidy queue.
    void (async () => {
        const db = createServerSupabase();
        try {
            await enqueueDbJob(db, {
                kind: "workflow.catalog_sync",
                payload: {},
                dedupeKey: "workflow.catalog_sync",
                maxAttempts: 5,
            });
        } catch (err) {
            console.error(
                "[workflow-catalog] boot sync enqueue failed; syncing inline:",
                err instanceof Error ? err.message : err,
            );
            await syncWorkflowAddonCatalog(db).catch((syncErr) =>
                console.error("[workflow-catalog] boot sync failed", syncErr),
            );
        }
    })();
}
