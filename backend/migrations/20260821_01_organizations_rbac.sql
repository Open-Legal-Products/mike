...
-- ---------------------------------------------------------------------------
-- Extends the signup trigger from 20260819_01_auth_email_confirmation (profile
-- row + email mirror). Still swallows errors (returns new) so a failed org
-- insert never blocks signup — the backfill migration is idempotent and will
-- repair any user left without a personal org.
...
-- upgraded deployment's backend could not touch the org tables at all.
...
grant select, insert, update, delete
  on public.organizations, public.org_members, public.teams, public.team_members
  to service_role;
