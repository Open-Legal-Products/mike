...
begin
  ...
  loop
    if nullif(trim(item->>'default_key'), '') is null then continue; end if;
    if exists (
      select 1 from public.default_workflow_installations dwi
    ...
    from jsonb_array_elements_text(
      case when jsonb_typeof(item->'jurisdictions') = 'array'
        then item->'jurisdictions' else '["General"]'::jsonb end
    );
    ...
    insert into public.workflows (
      ...
      p_user_id, item->>'title', item->>'type', nullif(item->>'prompt_md', ''),
    ...
    insert into public.default_workflow_installations(user_id, default_key, workflow_id)
    values (p_user_id, item->>'default_key', workflow_uuid);

    insert into public.quick_actions(
      user_id, workflow_id, prompt, document_upload, enabled, sort_order
    ) values (
      p_user_id, workflow_uuid, coalesce(item->>'quick_action_prompt', ''),
      coalesce((item->>'document_upload')::boolean, false), true,
