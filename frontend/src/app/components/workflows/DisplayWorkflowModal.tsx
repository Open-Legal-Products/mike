...
export function DisplayWorkflowModal({ workflows, workflow, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    // Re-sync only when the workflow identity changes, not on every field edit.
  ...
  useEffect(() => {
    ...
    // Only scroll when the selected row identity changes; later edits shouldn't re-scroll.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [selected?.id]);
          ...
          <>
            <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
              ...
              <div className="px-5 pb-3 shrink-0">
                <p className="text-xs font-medium text-gray-700 mb-2">Message (optional)</p>
                <textarea
                  rows={3}
