...
export function DocFileIcon({ fileType }: { fileType: string | null }) {
  if (fileType === "pdf") return <FileText className="h-3.5 w-3.5 text-red-500 shrink-0" />;
...
export function FileDirectory({
  ...
  if (allDocs.length === 0 && directoryProjects.length === 0 && uploadingFilenames.length === 0) {
    return <p className="text-center text-sm text-gray-400 py-8">{emptyMessage}</p>;
  }
      ...
      <div>
        ...
        {uploadingFilenames.map((filename) => (
          ...
          >
            ...
            <span className="shrink-0 text-gray-300">Uploading</span>
          </div>
        ))}
          ...
          return (
            <div key={project.id}>
              ...
              >
                ...
                <span className="text-xs text-gray-400 shrink-0">{docs.length}</span>
              </button>
              {isExpanded && (
                <div>
                  {docs.length === 0 ? (
                    <p className="pl-7 py-1 text-xs text-gray-400">Empty</p>
                  ) : (
                    docs.map((doc) => {
                      const selected = selectedIds.has(doc.id);
                      return (
