...
export function ProjectPage({ projectId, initialTab = "documents" }: Props) {
  ...
  const tab: ProjectTab =
    tabParam === "assistant" || tabParam === "reviews" ? tabParam : initialTab;
  const [addDocsOpen, setAddDocsOpen] = useState(false);
  const [peopleModalOpen, setPeopleModalOpen] = useState(false);
  const [ownerOnlyAction, setOwnerOnlyAction] = useState<string | null>(null);
  const { user } = useAuth();
  const [uploadVersionDoc, setUploadVersionDoc] = useState<MikeDocument | null>(null);
  ...
  const [versionsByDocId, setVersionsByDocId] = useState<Map<string, MikeDocumentVersion[]>>(
    () => new Map(),
  );
  ...
  async function submitNewVersion(doc: MikeDocument, file: File, displayName: string) {
    try {
    ...
    try {
      ...
      setProject((prev) =>
        prev && previous
          ? {
              ...prev,
              documents: (prev.documents ?? []).map((d) => (d.id === docId ? previous : d)),
            }
          : prev,
      );
    }
  ...
  function renderUploadingDocumentRows(depth: number) {
    return uploadingDroppedFilenames.map((filename) => (
      ...
      >
        ...
        <div className="w-24 shrink-0 text-sm text-gray-300">Uploading</div>
                ...
                {(() => {
                  ...
                  return (
                    <>
                      ...
                      <div
                        className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} bg-white p-2 group-hover:bg-gray-50`}
                        style={treeNameCellStyle(depth)}
                      >
                        <div className="flex items-center gap-2">
                          {isProcessing ? (
                          ...
                          {renamingDocumentId === doc.id ? (
                            <input
                              ...
                              onKeyDown={(e) => {
                                if (e.key === "Enter") void submitDocumentRename(doc.id);
                                if (e.key === "Escape") {
                                  setRenamingDocumentId(null);
          ...
          return (
            <div key={`folder-${folder.id}`}>
              ...
              >
                ...
                <div
                  className={`sticky left-8 z-[60] ${DOC_NAME_COL_W} p-2 ${dragOverFolderId === folder.id ? "bg-blue-50" : "bg-white"} group-hover:bg-gray-50`}
                  style={treeNameCellStyle(depth)}
                >
                  <div className="flex items-center gap-1.5">
                    {isExpanded ? (
            ...
            <div className="flex-1 flex flex-col min-h-0">
              ...
              <div
                ...
                onDrop={(e) => {
                  ...
                  setDragOverRoot(false);
                  setDragOverFolderId(null);
                  void handleDropProjectFiles(Array.from(e.dataTransfer.files));
                }}
              >
                      ...
                      >
                        {menuDoc ? (
                          <RowActionMenuItems
                            ...
                            onDelete={() => void handleRemoveDoc(menuDoc.id)}
                          />
                        ) : (
                          <RowActionMenuItems
                            onClose={() => setContextMenu(null)}
