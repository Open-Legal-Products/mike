...
import { Download, File, FileText, Loader2, Pencil, Plus, Users } from "lucide-react";
...
export function DocIcon({ fileType }: { fileType: string | null }) {
  if (fileType === "pdf") return <FileText className="h-4 w-4 text-red-600 shrink-0" />;
            ...
            >
              <div className="flex items-center gap-2">
                ...
                {isEditing ? (
                  ...
                  <span className="font-medium text-gray-700 truncate">{displayLabel}</span>
                )}
...
export function ProjectPageHeader({
  ...
  return (
    <div className="mb-1 flex items-start justify-between px-4 py-3 md:px-10">
      ...
      <div className="flex items-center gap-4">
        <HeaderSearchBtn value={search} onChange={onSearchChange} placeholder="Search…" />
        <button
        ...
        <div className="relative group">
          <button
            onClick={() => docsCount > 0 && !creatingReview && onNewReview()}
            className={`flex h-8 items-center justify-center gap-1.5 text-sm transition-colors ${
              docsCount > 0
                ? "text-gray-500 hover:text-gray-900 cursor-pointer"
                : "text-gray-300 cursor-default"
