...
export default function SignupPage() {
  ...
  const handleConfirm = async (e: React.FormEvent) => {
    ...
    if (signInError || !signInData.session) {
      setError(
        signInError?.message ?? "Confirmed, but could not sign in automatically. Please log in.",
      );
      setLoading(false);
      return;
    }
  ...
  const handleResend = async () => {
    setError(null);
    const { error: resendError } = await supabase.auth.resendConfirmationCode({ email });
      ...
      <div className="min-h-dvh bg-white flex items-start justify-center px-6 pt-32 md:pt-40 pb-10 relative">
        ...
        <div className="w-full max-w-md">
          <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-4">
            <h2 className="text-left text-2xl font-serif mb-2">Confirm your email</h2>
    ...
    <div className="min-h-dvh bg-white flex items-start justify-center px-6 pt-32 md:pt-40 pb-10 relative">
      ...
      <div className="w-full max-w-md">
        <div className="bg-white border border-gray-200 rounded-2xl p-8 mb-4">
          <div className="flex justify-between items-center mb-6">
            ...
            <div className="bg-gray-100 p-1 rounded-md flex text-xs font-medium">
              <Link href="/login" className="px-3 py-1 text-gray-500 hover:text-gray-900">
                Log in
              </Link>
        ...
        <p className="text-center text-xs text-gray-500 leading-relaxed px-2">
          Mike hosted on MikeOSS.com is currently a demo service. Please do not upload, submit, or
          store sensitive, confidential, privileged, client, or personally identifiable documents.
        </p>
      </div>
    </div>
  );
