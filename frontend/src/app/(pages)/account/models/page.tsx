...
import { isModelAvailable, modelGroupToProvider, providerLabel } from "@/app/lib/modelAvailability";
    ...
    <div className="space-y-4">
      ...
      <div className="py-6">
        ...
        <p className="text-sm text-gray-500 mb-4 max-w-xl">
          You must provide your own API keys for the app to work or add your API keys into the .env
          file if you are running your own instance of Mike.
        </p>
        <p className="text-xs text-gray-400 mb-4 max-w-xl">
          Title generation automatically routes to the cheapest configured provider model.
        </p>
        <div className="space-y-4 max-w-xl">
          {API_KEY_FIELDS.map((field) => (
            <ApiKeyField
              ...
              hasSavedKey={!!profile?.apiKeys[field.provider].configured}
              isServerConfigured={profile?.apiKeys[field.provider].source === "env"}
              onSave={(value) => updateApiKey(field.provider, value.trim() || null)}
              ...
              {items.map((m) => {
                ...
                return (
                  ...
                  >
                    ...
                    {!available && <AlertCircle className="h-3.5 w-3.5 text-red-500 ml-1" />}
  ...
  return (
    <div>
      ...
      {hasSavedKey && !isServerConfigured && (
        <p className="text-xs text-gray-500 mb-2">A key is saved. Paste a new key to replace it.</p>
      )}
      <div className="flex gap-2">
        ...
        {hasSavedKey && !isServerConfigured && (
          <Button type="button" variant="outline" onClick={handleRemove} disabled={isSaving}>
            Remove
          </Button>
        )}
      </div>
