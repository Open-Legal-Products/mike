...
export default function WorkflowDetailPage({ params }: Props) {
  ...
  const initialBuiltin = isBuiltin ? (BUILT_IN_WORKFLOWS.find((w) => w.id === id) ?? null) : null;

  const [workflow, setWorkflow] = useState<MikeWorkflow | null>(initialBuiltin);
  ...
  const [columns, setColumns] = useState<ColumnConfig[]>(initialBuiltin?.columns_config ?? []);
