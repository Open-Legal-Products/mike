...
// chatContext pulls in env/supabase at import; stub them so the pure spotlight
// helper can be tested in isolation.
vi.mock("../env", () => ({ env: { NODE_ENV: "test" } }));
vi.mock("../supabase", () => ({ createServerSupabase: () => ({}) }));

import { spotlight, generateSpotlightNonce } from "../chatContext";
