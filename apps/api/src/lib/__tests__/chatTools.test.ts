...
describe("generateSpotlightNonce", () => {
    it("returns a 32-character hex string (16 random bytes → 32 hex chars)", () => {
        const nonce = generateSpotlightNonce();
        expect(nonce).toMatch(/^[0-9a-f]{32}$/);
    });

    it("returns a different value on each call (collision would be a broken PRNG)", () => {
