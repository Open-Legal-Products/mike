...
async function getBulkCitationLookup(args: {
    ...
    for (const row of parsed) {
        ...
        const matchedClusters = clusterIds
            .map(
                (clusterId) =>
                    clustersById.get(clusterId) as
                        | ReturnType<typeof compactBulkCluster>
                        | undefined,
            )
            .filter(
                (
                    cluster: ReturnType<typeof compactBulkCluster> | undefined,
                ): cluster is ReturnType<typeof compactBulkCluster> =>
