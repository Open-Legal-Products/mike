-- Mike local PostgreSQL schema
-- Used when AUTH_MODE=local (Docker Compose self-hosted setup).
-- No Supabase auth schema, no RLS — the backend enforces access control.
...
create table if not exists public.user_api_keys (
  ...
  provider text not null check (provider in ('claude', 'gemini', 'openai', 'ollama')),
