...
- `apps/api/` - Express API, Supabase access, document processing, and database schema
...
- `apps/api/schema.sql` - Supabase schema for fresh databases
...
The schema file is based on `supabase-migration.sql` and folds in the later files in `supabase/migrations/`.

For an existing database, do not run the full schema file over production data. Apply the incremental files in `supabase/migrations/` instead.
...
2. If you did not set provider keys in `apps/api/.env`, open **Account > Models & API Keys** and add an Anthropic, Gemini, or OpenAI API key.
...
**The model picker shows a missing-key warning.** Add a key for that provider in **Account > Models & API Keys**, or configure the provider key in `apps/api/.env` and restart the backend.
