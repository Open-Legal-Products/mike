...
const normalized = catalogue.map((entry) => {
    ...
    if (entry.status !== "draft-awaiting-lawyer-review")
        throw new Error(`${entry.slug} cannot be published as approved.`);
    if (entry.reviewer !== null || entry.reviewDate !== null)
        throw new Error(
            `${entry.slug} has not completed external legal review.`,
        );
    if (
...
const backendWorkflows = normalized.map((entry) => ({
    ...
    id: entry.id,
    metadata: {
        title: `${entry.title} (Draft — not lawyer-reviewed)`,
        description: `DRAFT — not lawyer-reviewed. ${entry.description}`,
        type: "assistant",
        contributors: [
            {
...
console.log(
    `${checkOnly ? "Verified" : "Generated"} ${normalized.length} Ontario workflow drafts.`,
);
